...
{{- if .Values.dags.s3Sync.enabled }}
  ...
  {{ required "If `dags.s3Sync.enabled=true`, then `dags.persistence` must be enabled!" nil }}
  {{- end }}
  ...
  {{ required "If `dags.s3Sync.enabled=true`, then `dags.s3Sync.keyName` must be non-empty!" nil }}
  {{- end }}
