...
- name: dags-s3-initial-sync
  ...
  env:
{{/*
Need to define, if you have local K8s deployment, and not utilizing EKS IAM access
...
*/}}
  ...
  # Copy all files from s3 to the dags folder and grant permissions to the airflow user
    - /bin/sh
    - -c
    - aws s3 cp s3://${AWS_BUCKET}/${KEY_PATH} ${DEST_PATH} --exclude "*.pyc" --recursive
  volumeMounts:
    - name: dags-data
...
EXAMPLE USAGE: {{ include "airflow.container.s3_regular_sync" (dict "Release" .Release "Values") }}
*/}}
{{- define "airflow.container.s3_regular_sync" }}
- name: dags-s3-sync
  image: {{ .Values.dags.s3Sync.image.repository }}:{{ .Values.dags.s3Sync.image.tag }}
  imagePullPolicy: {{ .Values.dags.s3Sync.image.pullPolicy }}
  resources:
